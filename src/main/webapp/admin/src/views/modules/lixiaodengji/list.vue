                ...
                <el-row class="ad" :style="{justifyContent:contents.btnAdAllBoxPosition=='1'?'flex-start':contents.btnAdAllBoxPosition=='2'?'center':'flex-end'}">
                    <el-form-item>
                        ...
                        <a style="text-decoration:none" class="el-button el-button--success"
                           v-if="isAuth('lixiaodengji','导入导出')"
                           icon="el-icon-download"
                           href="http://localhost:8080/keshihuagongyu/upload/lixiaodengjiMuBan.xls"
                        >批量导入离校登记数据模板</a>
                        &nbsp;
                        <el-upload
                                v-if="isAuth('lixiaodengji','导入导出')"
                                ...
                                :on-success="lixiaodengjiUploadSuccess"
                                :on-error="lixiaodengjiUploadError"
                                :show-file-list = false>
                            <el-button
                                    v-if="isAuth('lixiaodengji','导入导出')"
                                    type="success"
                                    icon="el-icon-upload2"
                            >批量导入离校登记数据</el-button>
                        </el-upload>
                        &nbsp;
                        <!-- 导出excel -->
                        <download-excel v-if="isAuth('lixiaodengji','导入导出')" style="display: inline-block" class = "export-excel-wrapper" :data = "dataList" :fields = "json_fields" name = "lixiaodengji.xls">
                            <!-- 导出excel -->
                            <el-button
                                    type="success"
                                    icon="el-icon-download"
            ...
            <div class="table-content">
                <el-table class="tables" :size="contents.tableSize" :show-header="contents.tableShowHeader"
                    ...
                    <el-table-column width="300" :align="contents.tableAlign"
                                     ...
                                     label="操作">
                        <template slot-scope="scope">
                            <el-button v-if="isAuth('lixiaodengji','查看')" type="success" icon="el-icon-tickets" size="mini" @click="addOrUpdateHandler(scope.row.id,'info')">详情</el-button>
                            <el-button v-if="isAuth('lixiaodengji','修改')" type="primary" icon="el-icon-edit" size="mini" @click="addOrUpdateHandler(scope.row.id)">修改</el-button>
                            <el-button v-if="isAuth('lixiaodengji','删除')" type="danger" icon="el-icon-delete" size="mini" @click="deleteHandler(scope.row.id)">删除</el-button>
    ...
    //当前表下拉框搜索条件
        ...
        methods: {
            chartDialog() {
                ...
                //         groupColumn: "lixiaodengji_types",
                //     }
                //     this.$http({
            ...
            getDataList() {
                ...
                params['lixiaodengjiDelete'] = 1// 逻辑删除字段 1 未删除 2 删除


                this.$http({
                    url: "lixiaodengji/page",
                    method: "get",
                    params: params
                }).then(({data}) => {
                    if(data && data.code === 0){
            ...
            // 导入功能上传文件成功后调用导入方法
            lixiaodengjiUploadSuccess(data){
                let _this = this;
                _this.$http({
                    url: "lixiaodengji/batchInsert?fileName=" + data.file,
                    method: "get"
                }).then(({data}) => {
                    if(data && data.code === 0){
                        _this.$message({
